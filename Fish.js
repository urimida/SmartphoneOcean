...
class Fish {
  constructor() {
    ...
    this.planktons = [];
    const planktonCount = 8; // 기본 플랑크톤 개수 (성능 최적화: 15 -> 8)
  ...
  drawTextDetail() {
    ...
    // 제목에 제품명 포함
    let titleText = "쇼핑 물고기";
    if (this.shoppingData && this.shoppingData.productName) {
      const productName = this.shoppingData.productName;
      // 제품명이 길면 앞부분만 사용 (최대 15자)
      const shortProductName = productName.length > 15 ? productName.substring(0, 15) + "..." : productName;
      titleText = `쇼핑 물고기: ${shortProductName}`;
    }
    ...
    const animalRadius = max(fishDisplayWidth, fishDisplayHeight) / 2;
    
    // 텍스트 모달이 열려있을 때는 손 위치와 마우스 위치 둘 다 확인
    const isTextDetailOpen = typeof showShoppingDetail !== 'undefined' && showShoppingDetail;
    ...
    if (shouldShowText && this.shoppingData) {
      // 호버 시작 시간 기록
      ...
      // 핵심 문장 추출
      const keyPhrases = [];
      if (this.shoppingData.storeName) keyPhrases.push(this.shoppingData.storeName);
      if (this.shoppingData.productName) {
        const product = this.shoppingData.productName;
        if (product.length > 25) {
          keyPhrases.push(product.substring(0, 25) + "...");
        } else {
          keyPhrases.push(product);
        }
      }
      if (this.shoppingData.category) keyPhrases.push(this.shoppingData.category);
      if (this.shoppingData.price) {
        keyPhrases.push(`${this.shoppingData.price.toLocaleString()}원`);
      }
    ...
    // -------------------------
    // 닫기 버튼 (물고기 아래) - 20픽셀 위로 올림
    // -------------------------
    const btnY = cy + headOffsetY + fishDisplayHeight / 2 + 95; // 115 -> 95 (20픽셀 위로)
    const btnW = 140;
    const btnH = 35;
